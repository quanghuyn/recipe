import View from './View.js'

import incons from 'url:../../img/icons.svg'
import { Fraction } from 'fractional'
  class RecipeView extends View {
    _parentE = document.querySelector('.recipe')
    _data
    _ErroMessage = ' Can`t load recipe. Try again'
    render(data) {
        this._data = data
        const markup = this._generateMarkup()
        this.clear()
        this._parentE.insertAdjacentHTML('afterbegin', markup)
    }

    clear(){
        this._parentE.innerHTML = ' '
    }
    addHandlerBookMark(handler){
      this._parentE.addEventListener('click',(e) => {
        const bookMark = e.target.closest('.btn--round')
        if(!bookMark) return
        handler()
        
      })
    }
    _generateMarkup() {

        return `
        <figure class="recipe__fig">
          <img src="${this._data.imageurl}" alt="${this._data.title}" class="recipe__img" />
          <h1 class="recipe__title">
            <span>${this._data.title}</span>
          </h1>
        </figure>
      
        <div class="recipe__details">
          <div class="recipe__info">
            <svg class="recipe__info-icon">
              <use href="${incons}#icon-clock"></use>
            </svg>
            <span class="recipe__info-data recipe__info-data--minutes">${this._data.cookingtime}</span>
            <span class="recipe__info-text">minutes</span>
          </div>
          <div class="recipe__info">
            <svg class="recipe__info-icon">
              <use href="${incons}#icon-users"></use>
            </svg>
            <span class="recipe__info-data recipe__info-data--people">${this._data.servings}</span>
            <span class="recipe__info-text">servings</span>
      
            <div class="recipe__info-buttons">
              <button class="btn--tiny btn--increase-servings"data-update ="${this._data.servings - 1}">
                <svg>
                  <use href="${incons}#icon-minus-circle"></use>
                </svg>
              </button>
              <button class="btn--tiny btn--increase-servings" data-update ="${this._data.servings + 1}">
                <svg>
                  <use href="${incons}#icon-plus-circle"></use>
                </svg>
              </button>
            </div>
          </div>
      
          <div class="recipe__user-generated ${this._data.key ? '': 'hidden'}">
            <svg>
              <use href="${incons}#icon-user"></use>
            </svg>
          </div>
          <button class="btn--round">
            <svg class="">
              <use href="${incons}#icon-bookmark${this._data.bookMarkSate ? '-fill' : ''}"></use>
            </svg>
          </button>
        </div>
      
        <div class="recipe__ingredients">
          <h2 class="heading--2">Recipe ingredients</h2>
          <ul class="recipe__ingredient-list">
           ${this._data.ingredients.map( e =>{
              return`
                 <li class="recipe__ingredient">
                   <svg class="recipe__icon">
                     <use href="${incons}#icon-check"></use>
                   </svg>
                    <div class="recipe__quantity">${e.quantity ?  new Fraction(e.quantity).toString() : ' ' }</div>
                    <div class="recipe__description">
                    <span class="recipe__unit">${e.unit}</span>
                    ${e.description}
                   </div>
                  </li>
              `
             }).join('')}
          </ul>
        </div>
      
        <div class="recipe__directions">
          <h2 class="heading--2">How to cook it</h2>
          <p class="recipe__directions-text">
            This recipe was carefully designed and tested by
            <span class="recipe__publisher">${this._data.publisher}</span>. Please check out
            directions at their website.
          </p>
          <a
            class="btn--small recipe__btn"
            href="${this._data.sourceurl}"
            target="_blank"
          >
            <span>Directions</span>
            <svg class="search__icon">
              <use href="${incons}#icon-arrow-right"></use>
            </svg>
          </a>
        </div>
          `
    }

    renderError(message = this._ErroMessage){
      const markup = `<div class="error">
        <div>
          <svg>
            <use href="${incons}#icon-alert-triangle"></use>
          </svg>
        </div>
        <p>${message}</p>
      </div>`
      this.clear()
      this._parentE.insertAdjacentHTML('afterbegin', markup)
    }

    loadSpiner = function(){
        let markup = `<div class="spinner">
        <svg>
          <use href="${incons}#icon-loader"></use>
        </svg>
      </div>`
        this._parentE.innerHTML = ' '
        this._parentE.insertAdjacentHTML('afterbegin', markup )
    }

    addHandlerRender(handler) {
      window.addEventListener('hashchange', handler)
      window.addEventListener('load', handler)
    }
    addHandlerUpdateServing(handler){
        this._parentE.addEventListener('click',function(e){
         const btn =  e.target.closest('.btn--tiny')
         if(!btn) return
         const  newServing = +btn.dataset.update
         handler(newServing)
        })
       
       }

      
}
export default new RecipeView()